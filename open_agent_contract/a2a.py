    ...
    *,
    ...
    if "max_amount" in constraints:
        ...
        if raw_amount is None:
            issues.append(ConformanceIssue(code="amount_missing", message="transaction amount required by max_amount constraint"))
        else:
            try:
                amount = Decimal(str(raw_amount))
                maximum = Decimal(str(constraints["max_amount"]))
            except (InvalidOperation, ValueError):
                issues.append(ConformanceIssue(code="amount_invalid", message="transaction amount or max_amount is not numeric"))
            else:
                if amount > maximum:
                    issues.append(ConformanceIssue(code="amount_exceeds_mandate", message=f"amount {amount} exceeds contract ceiling {maximum}"))

    if "currency" in constraints:
        currency = intent.attributes.get("currency")
        if currency is None:
            issues.append(ConformanceIssue(code="currency_missing", message="transaction currency required by contract"))
        elif str(currency).upper() != str(constraints["currency"]).upper():
            issues.append(ConformanceIssue(code="currency_mismatch", message="transaction currency does not match contract currency"))
